    ...
    <body>
        ...
        <div class="container-fluid">
            <div class="col ms-5 me-5" style="margin-top: 5%">
                <div class="row">
                    <h4 class="text-center">DỊCH VỤ HTTP</h4>
                </div>
                <!--dich vu http-->
                <div id="http" class="row bg-white p-3  border-top"> 

                    <div class="col-5 ms-auto">
                        <div class="card border-0 shadow-sm">
                            <div class="card-header text-center">
                                <h5>File cấu hình dịch vụ HTTP</h5>
                            </div>
                            <div class="card-body">
                                <form action="configHTTP" method="post">
                                    ...
                                    <div id="fileHTTP" style="width: 100%; max-height: 400px; overflow-y: auto;">
                                        <% int http = 1;%>
                                        <c:forEach var="line" items="${seeHTTP}">
                                            <div class="lh-lg">
                                                <c:choose>
                                                    <c:when test="${fn:contains(line, '0') || fn:contains(line, '1') || fn:contains(line, '2') || fn:contains(line, '3') || 
                                                            ...
                                                            <div class="d-flex align-items-center border-bottom">
                                                                <label class="text-danger me-3" for="http<%=http%>" >${fn:substring(line, 0, fn:indexOf(line, ' ')+1)}</label>
                                                                <input type="hidden" name="line0" value="${fn:substring(line, 0, fn:indexOf(line, ' ')+1)}">
                                                                <input id="http<%=http++%>" class="text-danger border-0 w-100" type="text" name="line1" value='${fn:substring(line, fn:indexOf(line, ' ')+1, fn:length(line))}'>
                                                            </div>
                                                    </c:when>
                                                    <c:otherwise>
                                                        <div class="d-flex align-items-center border-bottom">
                                                            <label class="me-3" for="http<%=http%>" >${fn:substring(line, 0, fn:indexOf(line, ' ')+1)}</label>
                                                            <input type="hidden" name="line0" value="${fn:substring(line, 0, fn:indexOf(line, ' ')+1)}">
                                                            <input id="http<%=http++%>" class="border-0 w-100" type="text" name="line1" value='${fn:substring(line, fn:indexOf(line, ' ')+1, fn:length(line))}'>
                                                        </div>
                                                    </c:otherwise>
                                                </c:choose>
                                            </div>
                    ...
                    <div class="col-5 me-auto">  
                        <div class="card border-0 shadow-sm mb-5">
                            <div class="card-header text-center pt-2">
                                <h5>Điều khiển dich vụ HTTP</h5>
                            </div>
                            <div class="card-body">
                                <form action="configHTTP" method="post">
                                    <div class="btn-group d-flex justify-content-sm-around">
                                        <button class="btn btn-success me-2" type="submit" name="control" value="start">Start HTTP</button>
                                        <button class="btn btn-warning me-2" type="submit" name="control" value="restart">Restart HTTP</button>
                                        <button class="btn btn-info me-2" type="submit" name="control" value="status">Status HTTP</button>
                                        <button class="btn btn-danger me-2" type="submit" name="control" value="stop">Stop HTTP</button>
                                    </div>
                                    <input type="hidden" name="host" value="<%=u.getHost()%>">
                                    <input type="hidden" name="port" value="<%=u.getPort()%>">
                                    <input type="hidden" name="user" value="<%=u.getUser()%>">
                                ...
                                <form action="configHTTP" method="post">
                                    ...
                                    <table class="table">
                                        ...
                                        <tr class="border-bottom">
                                            ...
                                            <td><button class="btn btn-sm btn-success" type="submit" name="control" value="mysql">OK</button></td>
                                        </tr>
                                        <tr>
                                            <td>2</td>
                                            <td>PHP</td>
                                            ...
                                            <td><input type="radio" name="choose" value="reinstall"></td>
                                            <td><input type="radio" name="choose" value="remove"></td>
                                            <td><button class="btn btn-sm btn-success" type="submit" name="control" value="php">OK</button></td>
                                        </tr>
                                    </table>
                                </form>
                            </div>
